...
namespace slam {
    namespace se3 {
        ...
        Eigen::Matrix4d hat(const Eigen::Ref<const Eigen::Vector3d>& rho, 
            ...
            mat.topLeftCorner<3, 3>() = slam::so3::hat(aaxis);
        ...
        Eigen::Matrix<double, 6, 6> curlyhat(const Eigen::Ref<const Eigen::Vector3d>& rho, 
            ...
            mat.topLeftCorner<3, 3>() = mat.bottomRightCorner<3, 3>() = slam::so3::hat(aaxis);
            mat.topRightCorner<3, 3>() = slam::so3::hat(rho);
            return mat;
        }
        ...
        Eigen::Matrix<double, 4, 6> point2fs(const Eigen::Ref<const Eigen::Vector3d>& p, double scale) noexcept {
            ...
            mat.topRightCorner<3, 3>() = -slam::so3::hat(p);
            return mat;
        }
        ...
        Eigen::Matrix<double, 6, 4> point2sf(const Eigen::Ref<const Eigen::Vector3d>& p, double scale) noexcept {
            ...
            mat.bottomLeftCorner<3, 3>() = -slam::so3::hat(p);
        ...
        void vec2tran_analytical(const Eigen::Ref<const Eigen::Vector3d>& rho_ba,
            ...
            Eigen::Matrix3d J_ab;
            slam::so3::vec2rot(aaxis_ba, out_C_ab, &J_ab);
        ...
        void vec2tran_numerical(const Eigen::Ref<const Eigen::Vector3d>& rho_ba,
            ...
            Eigen::Matrix4d x_small = slam::se3::hat(xi_ba);
        ...
        Eigen::Matrix<double, 6, 1> tran2vec(const Eigen::Ref<const Eigen::Matrix3d>& C_ab,
            ...
            Eigen::Matrix<double, 6, 1> xi_ba;
            Eigen::Vector3d aaxis_ba = slam::so3::rot2vec(C_ab);
            Eigen::Vector3d rho_ba = slam::so3::vec2jacinv(aaxis_ba) * r_ba_ina;
            xi_ba << rho_ba, aaxis_ba;
            return xi_ba;
        }
        ...
        Eigen::Matrix<double, 6, 6> tranAd(const Eigen::Ref<const Eigen::Matrix3d>& C_ab,
            ...
            adjoint_T_ab.topRightCorner<3, 3>() = slam::so3::hat(r_ba_ina) * C_ab;
            return adjoint_T_ab;
        }
        ...
        Eigen::Matrix3d vec2Q(const Eigen::Ref<const Eigen::Vector3d>& rho_ba,
            ...
            if (ang < 1e-12) return 0.5 * slam::so3::hat(rho_ba);
            ...
            Eigen::Matrix3d rx = slam::so3::hat(rho_ba);
            Eigen::Matrix3d px = slam::so3::hat(aaxis_ba);
        ...
        Eigen::Matrix<double, 6, 6> vec2jac(const Eigen::Ref<const Eigen::Vector3d>& rho_ba,
            ...
            if (aaxis_ba.norm() < 1e-12) {
                ...
                J_ab.topRightCorner<3, 3>() = 0.5 * slam::so3::hat(rho_ba);
            } else {
                J_ab.topLeftCorner<3, 3>() = J_ab.bottomRightCorner<3, 3>() = slam::so3::vec2jac(aaxis_ba);
                J_ab.topRightCorner<3, 3>() = slam::se3::vec2Q(rho_ba, aaxis_ba);
            }
            return J_ab;
        }
        ...
        Eigen::Matrix<double, 6, 6> vec2jac(const Eigen::Ref<const Eigen::Matrix<double, 6, 1>>& xi_ba,
            ...
            Eigen::Matrix<double, 6, 6> x_small = slam::se3::curlyhat(xi_ba);
        ...
        Eigen::Matrix<double, 6, 6> vec2jacinv(const Eigen::Ref<const Eigen::Vector3d>& rho_ba,
            ...
            if (aaxis_ba.norm() < 1e-12) {
                ...
                J66_ab_inv.topRightCorner<3, 3>() = -0.5 * slam::so3::hat(rho_ba);
            } else {
                // General analytical scenario
                Eigen::Matrix3d J33_ab_inv = slam::so3::vec2jacinv(aaxis_ba);
                J66_ab_inv.topLeftCorner<3, 3>() = J66_ab_inv.bottomRightCorner<3, 3>() = J33_ab_inv;
                J66_ab_inv.topRightCorner<3, 3>() = -J33_ab_inv * slam::se3::vec2Q(rho_ba, aaxis_ba) * J33_ab_inv;
            }
            return J66_ab_inv;
        }
